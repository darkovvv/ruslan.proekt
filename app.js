...
const prizes = [
  { label: '-100₽', val: 100, type: 'discount', ...w },
  { label: 'Ничего', val: 0, type: 'none', ...b },
  { label: '-150₽', val: 150, type: 'discount', ...w },
  { label: 'Ничего', val: 0, type: 'none', ...b },
  { label: '-200₽', val: 200, type: 'discount', ...w },
  { label: '-500₽', val: 500, type: 'discount', ...w },
  { label: 'Ничего', val: 0, type: 'none', ...b },
  { label: '-1000₽', val: 1000, type: 'discount', ...w },
  { label: 'БИЛЕТ', val: 1, type: 'free', ...g },
  { label: '-100₽', val: 100, type: 'discount', ...w },
  { label: 'Ничего', val: 0, type: 'none', ...b },
  { label: '-150₽', val: 150, type: 'discount', ...w },
  { label: '-200₽', val: 200, type: 'discount', ...w },
...
/* формат цены */
const formatPrice = (price) => `${Math.max(0, Math.round(price)).toLocaleString('ru-RU')} ₽`;

/* расчет динамической скидки */
const calculateDynamicPrice = (base, fullPct) => base * (1 - (fullPct / 100) * 0.35);

/* функция цвета: 0-20 (красный), 20-60 (оранжевый), 60-100 (зеленый) с переходом яркости */
const getDynamicColor = (pct) => {
  let hue, lightness;
  if (pct <= 20) {
    hue = 0; lightness = 30 + (pct / 20) * 25; // от темно-красного к ярко-красному
  } else if (pct <= 60) {
    hue = 30; lightness = 35 + ((pct - 20) / 40) * 20; // от темного к светло-оранжевому
  } else {
    hue = 120; lightness = 25 + ((pct - 60) / 40) * 35; // от темного к ярко-зеленому
  }
...
function initDynamicDemo() {
  ...
  const updateDemo = () => {
    ...
    percentVal.style.textShadow = `0 0 15px ${color}`;
...
function renderTrips() {
  ...
  grid.innerHTML = staticTrips.map((trip, idx) => {
    ...
    return `
      <article class="card trip-card animate-up" style="transition-delay: ${idx * 0.1}s">
...
function initBookingSystem() {
  ...
  confirmBtn.addEventListener('click', () => {
    ...
    alert(`Успешно оформлено! Итоговая сумма: ${document.getElementById('bookingTotal').textContent}. Места: ${selectedSeatsArray.join(', ')}`);
...
function openBooking(tripId, currentPrice, currentFill) {
  if (!currentUser) { alert('Сначала войди в профиль!'); return; }
  ...
  if (currentUser.prize && currentUser.prize.type !== 'none') {
    ...
    prizeBtn.textContent = `Применить приз: ${currentUser.prize.label}`;
  } else {
...
function updateBookingTotal() {
  ...
  if (isDiscountApplied && currentUser.prize) {
    ...
    else if (currentUser.prize.type === 'free') total -= currentBookingTrip.price; // минус один билет
  }
  document.getElementById('bookingTotal').textContent = formatPrice(total);
}

/* постройка ровной рулетки */
function buildRoulette() {
  ...
  prizes.forEach((prize, i) => {
    ...
    // центр сектора. -90 градусов нужно, т.к. 0deg градиента = это "север" (12 часов), 
    // а 0deg трансформации CSS = это "восток" (3 часа).
  ...
  // применяем идеальный круговой градиент
  wheel.style.background = `conic-gradient(${gradientParts.join(', ')})`;

  document.getElementById('openRouletteBtn').addEventListener('click', (e) => {
    e.preventDefault();
    if (!currentUser) { alert('Войди или зарегистрируйся, чтобы крутить рулетку!'); return; }
    if (currentUser.played) { alert('Ты уже испытал удачу!'); return; }
  ...
  spinBtn.addEventListener('click', () => {
    ...
    // вращаем на 5 полных оборотов минус нужный угол
    ...
    setTimeout(() => {
      ...
      localStorage.setItem('fanTripUser', JSON.stringify(currentUser));
      alert(`Твой приз: ${prizes[targetIdx].label}! Забирай скидку при бронировании.`);
